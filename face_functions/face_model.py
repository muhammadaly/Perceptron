...
from face_functions.mtcnn_detector import MtcnnDetector
...
class FaceModel:
  ...
  def detect_faces(self, i_img):
    return self.detector.detect_face(i_img, det_type = self.args.det)

  def get_input(self, i_img, bbox, points):
    bbox = bbox[0:4]
    points = points.reshape((2, 5)).T
    nimg = preprocess(i_img, bbox, points, image_size='112,112')
